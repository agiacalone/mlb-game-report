...
### Dataset directory + zip bundle

Under the hood, `mlb-report` is a thin wrapper over `scripts/mlb-fetch` (API → CSV) and `scripts/mlb-render` (CSV → Markdown/HTML). Every run produces a companion directory of CSVs:

```
~/games-attended/2026-04-17-padres-at-angels/
├── game.csv, linescore.csv, batting.csv,
├── pitching.csv, plays.csv, pitches.csv, statcast.csv
```

`statcast.csv` is one row per ball in play, sourced from **Baseball Savant's
game feed** (`gf` endpoint) — it carries the expected stats and bat tracking
the MLB Stats API does not expose: `xba`, `bat_speed`, the authoritative
`is_barrel` flag, plus spray coords (`hc_x`/`hc_y`), EV/LA/distance and the
pitch that was hit. When Savant is unreachable the row set is rebuilt from the
feed/live `hitData` already in hand (no expected stats / bat speed; barrels
approximated, `source=feed` instead of `savant`). `pitches.csv` additionally
carries the pitch-movement profile (`ivb_in`, `hb_in`, `spin_dir`, `ext_ft`)
and Gameday landing coords (`hit_x`/`hit_y`).

The CSV dataset is the source of truth; the Markdown and HTML are reproducible views over it. Pass `--zip` to bundle the dataset directory + `.md` + `.html` into `<slug>.zip` for portability or download. For CSV-only workflows (e.g. feeding a separate skill), call `scripts/mlb-fetch` directly. To re-render without hitting the API (e.g. after editing the stylesheet or correcting a CSV), call `scripts/mlb-render <dataset-dir> --html`.
...
12. **⌖ STATCAST** — the analytics spread, three hand-drawn inline-SVG newspaper "clipping" charts plus two fact tables: a **spray chart** (engraved ballfield, dots tinted by `xBA`, barrels ringed, HRs pipped), an **EV × LA scatter** (hard-hit + sweet-spot zones shaded, barrels ringed), a **pitch-movement plot** (horizontal × induced-vertical break, coloured by pitch family), a **GAME LEADERS** ledger (hardest hit, longest HR, best barrel, fastest bat, fastest pitch, most spin, biggest break), and **EXPECTED vs. ACTUAL** (hard-hit outs that were "robbed" + soft hits that "found a hole", by `xBA`). Degrades gracefully when Savant is down: charts still render from feed/live data, expected-stat colour/columns drop out. The SVGs are self-contained (own cream backing) so they read identically in a dark Obsidian note and on the light newspaper HTML page.
13. **NOTES** — WP, balks, HBP, IBB, SB, CS, E, DP, pitch-timer violations, etc.
14. **PLAY-BY-PLAY** — every plate appearance itemized MLB Gameday-style under a half-inning header (`### Bottom 2nd — Angels batting · Padres pitching`). Each item: batter (vs. pitcher), event type, full MLB description, scoring tag, count/pitch count, and a Statcast line with last pitch type/velocity/spin and (for balls in play) EV/LA/distance/trajectory/fielded position. Mid-inning pitching changes annotated.
15. **PERSONAL NOTES** *(only if `--attended`)* — empty template section at the bottom for hand-editing memories.
...
- `GET https://statsapi.mlb.com/api/v1.1/game/{gamePk}/feed/live` (used for decisions + scoring plays + per-pitch `hitData`/`pitchData.breaks`)
- `GET https://statsapi.mlb.com/api/v1/game/{gamePk}/winProbability` (WPA / leverage)
- `GET https://baseballsavant.mlb.com/gf?game_pk={gamePk}` (Baseball Savant game feed — `exit_velocity[]` array supplies xBA / bat speed / barrel flags / spray coords for `statcast.csv`; best-effort, browser UA, degrades to feed/live `hitData` on any failure)
